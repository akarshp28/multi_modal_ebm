...
def get_imgs(dataset_name, option='load'):
    image_folder = '/' + dataset_name + '2014/'
    ...
    if option == 'DL_BOTH':
        ...
        IMGPATH = os.path.abspath('.') + image_folder
...
def make_gallery(data, ncols, epoch, name, save_path, save):
...
def make_img(data, ncols, epoch, save_path, save, name):
    data = data[:, 0, :, :, :]
#     data0 = tf.expand_dims(data[0], axis=0).numpy()
#     make_gallery(data0, 1, epoch, 'ld_orig_' + name, save_path, save)
    make_gallery(data[1:], ncols, epoch, 'ld_step_' + name, save_path, save)
...
def make_gallery2(test_data, model_en, model_de, mode, epoch, name, save_path, save, latent_dim, ncols=10, num_imgs=100):
    if mode == 'LD':
        x_d = []
        cum_energy = []
        cum_zmag = []
        ...
        make_img(ld_images, ncols, epoch, name, save_path, save)
    ...
    make_gallery(array, ncols, epoch, name, save_path, save)
...
def draw2(image, recon, save_path, save, epoch, name):
...
def draw3(image, recon, ld, save_path, save, epoch, name):
...
def create_image_gen(img_paths, batch_size, img_shape, buffer_size=10000, autotune=False):
    encode_train = sorted(set(img_paths))
    print('\ndataset size:', len(encode_train), flush=True)
