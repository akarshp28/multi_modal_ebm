...
if use_multigpu:
    ...
    dataset_name = 'train'
    train_capspath, train_imgspath = get_imgs(dataset_name)
    train_caps, train_imgs = load_caps_img(train_capspath, train_imgspath, dataset_name)
    
    dataset_name = 'val'
    val_capspath, val_imgspath = get_imgs(dataset_name)
    val_caps, val_imgs = load_caps_img(val_capspath, val_imgspath, dataset_name)

    dataset_name = 'test'
    test_capspath, test_imgspath = get_imgs(dataset_name)
    test_imgs = load_caps_img(test_capspath, test_imgspath, dataset_name)
...
if use_multigpu:
    ...
    with strategy.scope():
        ...
        @tf.function
        def distributed_train_step(dataset_inputs, step, epoch):
            return strategy.run(model.train_step, args=(dataset_inputs, step, epoch))
        
        @tf.function
        def distributed_test_step(dataset_inputs, step, epoch, mode):
            return strategy.run(model.test_step, args=(dataset_inputs, step, epoch, mode))
        ...
        print("Use full dataset: {}, Drop stray batches: {}, Multi_GPU: {}".format(use_all_data, drop_remain, use_multigpu))
