...
class Postprocess(tf.keras.Model):
    def __init__(self, n_blocks, n_cells, mult, n_channels_decoder, scale_factor, dataset_option, **kwargs) -> None:
        ...
        if dataset_option == 'mnist':
            self.sequence.add(SpectralNormalization(layers.Conv2D(1, kernel_size=(3, 3), padding="same")))
        else:
            self.sequence.add(SpectralNormalization(layers.Conv2D(100, kernel_size=(3, 3), padding="same"))) # 10 logistic distributions
