...
def save_samples_to_tensorboard(epoch, model, image_logger):
    for temperature in [0.7, 0.8, 0.9, 1.0]:
        ...
        images = interval_mapping(images.numpy(), -1.0, 1.0, 0.0, 1.0)
        
        with image_logger.as_default():
            tf.summary.image(f"t={temperature:.1f}", images, step=epoch)


def save_reconstructions_to_tensorboard(epoch, model, test_data: tf.data.Dataset, image_logger, dataset_option):
    print('Display Test Data', flush=True)
    
    if dataset_option == 'mnist':
        batch = tf.convert_to_tensor(next(test_data.shuffle(buffer_size=10).as_numpy_iterator())[0])
    else:
        batch = tf.convert_to_tensor(next(test_data.shuffle(buffer_size=10).as_numpy_iterator()))
