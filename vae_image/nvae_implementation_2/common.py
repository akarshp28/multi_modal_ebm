...
class Sampler(tf.keras.Model):
    ...
    def call(self, prior, z_idx, enc_prior=None) -> Tuple[tf.Tensor, DistributionParams]:
        ...
        if z_idx == 0:
            ...
            params = DistributionParams(enc_mu, enc_sigma, tf.zeros_like(enc_mu), tf.ones_like(enc_sigma))
            return z, params
...
class Rescaler(tf.keras.Model):
    ...
    def build(self, input_shape):
        ...
        if self.rescale_type == RescaleType.UP:
            self.conv = SpectralNormalization(layers.Conv2D(self.n_channels, (3, 3), strides=(1, 1), padding="same"))

        elif self.rescale_type == RescaleType.DOWN:
            self.conv = SpectralNormalization(layers.Conv2D(self.n_channels, (3, 3), strides=(self.scale_factor, self.scale_factor), padding="same"))
    ...
    def call(self, input):
        ...
        if self.rescale_type == RescaleType.UP:
            ...
            x = tf.image.resize(x, size=(self.scale_factor * height, self.scale_factor * width), method="nearest")
        x = self.conv(x)
        return x
