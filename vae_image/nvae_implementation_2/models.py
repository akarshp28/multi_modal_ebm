...
class NVAE(tf.keras.Model):
    def __init__(self,
        ...
        self.preprocess = Preprocess(n_encoder_channels, n_preprocess_blocks, n_preprocess_cells, scale_factor, input_shape)
    ...
    # process starts here # main method
    ...
    def train_step(self, data):
        ...
        reconstruction = self.sample_from_discretized_mix_logistic(reconstruction)
        mse = tf.reduce_mean(tf.reduce_sum(tf.keras.losses.mean_squared_error(data, reconstruction), axis=(1, 2)))
        return {"loss": total_loss, "reconstruction_loss": recon_loss, "kl_loss": kl_loss, "bn_loss": bn_loss, "mse_loss": mse}
    
    
    def sample(self, n_samples=16, temperature=1.0, greyscale=False):
    ...
    def discretized_mix_logistic_loss(self, x, l, sum_all=True):
        # check reconstructed data if in range
    ...
    def sample_from_discretized_mix_logistic(self, l, nr_mix=10):
        # l is the reconstruction vector
        ls = self.int_shape(l)
        xs = ls[:-1] + [3]
